  ...
  <v-container>
    <v-card outlined elevation="1" class="mb-6 filter-card">
      <div class="filter-card-header">
        ...
        <v-select class="year-select" :items="yearOptions" v-model="selectedYearValue" label="ปี" density="comfortable"
          hide-details @update:modelValue="onYearSelected"></v-select>
      </div>
      <v-card-text>
        <v-row class="gy-4">
          <v-col cols="12" md="4">
            <v-autocomplete label="Provider" :items="providers" v-model="selectedProvider" item-title="fullname"
              item-value="id" @update:modelValue="onProviderSelected" dense></v-autocomplete>
          </v-col>

          <v-col cols="12" md="4">
            <v-select class="full-width" :items="monthOptions" label="เดือน" v-model="selectedMonthName" clearable
              @update:modelValue="onMonthSelected"></v-select>
          </v-col>

          <v-col cols="12" md="4">
            <v-autocomplete label="Week" :items="weeks" item-text="display" item-title="label" v-model="selectedWeek"
              return-object @change="onWeekSelected" dense clearable></v-autocomplete>
          </v-col>
          ...
          <v-col cols="12">
            <div class="filter-actions">
              ...
              <v-btn color="primary" variant="tonal" class="filter-action-btn" @click="resetSelections">
                Clear
              </v-btn>
            </div>
          </v-col>
    ...
    <v-card outlined elevation="1" class="pa-4 mb-6 settlement-card">
      ...
      <v-card-text>
        <div class="settlement-create-trigger">
          ...
          <v-btn color="primary" :disabled="!canOpenCreateSettlement" @click="openCreateSettlementDialog">
            สร้างการชำระบิล
          </v-btn>
        </div>
        ...
        <div class="settlement-list-header">
          ...
          <v-switch class="use-paid-at-switch" v-model="usePaidAt" inset hide-details color="primary"
            :label="usePaidAt ? 'ใช้ Paid At' : 'ใช้ Bill Date'" @update:modelValue="handleUsePaidAtUpdate"></v-switch>
        </div>

        <v-skeleton-loader v-if="billSettlementsLoading" type="list-item-three-line" class="mt-4"></v-skeleton-loader>

        <v-alert v-else-if="!billSettlements.length" type="info" variant="tonal" class="mt-4">
          ไม่มีการชำระบิลสำหรับเดือนนี้
        </v-alert>

        <v-list v-else class="bill-settlement-list mt-4" density="comfortable">
          <v-list-item v-for="settlement in billSettlements" :key="settlement.id"
            @click="openEditSettlement(settlement)" class="bill-settlement-item">
            <v-list-item-title>
    ...
    <v-card outlined elevation="2" class="pa-4">
      <div class="summary-header">
        ...
        <v-btn @click="exportExcel" color="#1d6f42" variant="outlined" :disabled="!showItem.length">
          <v-icon class="mr-2">mdi-microsoft-excel</v-icon>ส่งออก Excel
        </v-btn>
      </div>

      <div class="text-h6 mt-2">
        รวม {{ totalHours }} ชั่วโมง {{ totalRealPrice }} บาท
      </div>

      <v-divider class="my-4"></v-divider>

      <v-data-table-virtual :loading="tableLoading" :headers="headers" :items="showItem" height="400" item-value="name"
      ...
      <v-card>
        ...
        <v-card-text>
          <v-form @submit.prevent="handleCreateSettlement">
            <v-row class="gy-4">
              <v-col cols="12" md="6">
                <v-text-field label="Provider" :model-value="getSelectedProviderName || ''" disabled></v-text-field>
              </v-col>
              <v-col cols="12" md="6">
                <v-text-field label="Settle Date" v-model="settlementForm.settle_date" disabled></v-text-field>
              </v-col>
              <v-col cols="12" md="6">
                <v-text-field label="Paid At" v-model="settlementForm.paid_at" type="datetime-local" :max="maxPaidAt"
                  required></v-text-field>
              </v-col>
              <v-col cols="12" md="6">
                <v-select label="Bank" :items="bankOptions" item-title="label" item-value="id"
                  v-model="settlementForm.bank_id" clearable required></v-select>
              </v-col>
              <v-col cols="12" md="6">
                <v-text-field label="Amount" v-model="settlementForm.amount" type="number" step="0.01" min="0"
                  required></v-text-field>
              </v-col>
            </v-row>
          </v-form>
        ...
        <v-card-actions class="justify-end">
          <v-btn variant="text" :disabled="creatingBillSettlement" @click="closeCreateSettlementDialog">
            ยกเลิก
          </v-btn>
          <v-btn color="primary" :disabled="!canCreateSettlement || creatingBillSettlement"
            :loading="creatingBillSettlement" @click="handleCreateSettlement">
            Create
          </v-btn>
        </v-card-actions>
      </v-card>
      ...
      <v-card>
        ...
        <v-card-text class="py-0">
          <div class="bill-payment-section">
            <v-data-table v-if="billPaymentTableItems.length" :headers="billPaymentHeaders"
              :items="billPaymentTableItems" class="bill-payment-table" density="comfortable" :items-per-page="-1"
              hide-default-footer>
              <template #item.actions="{ item }">
                <v-btn variant="text" size="small" color="primary" @click="openBillPaymentDialog('edit', item.raw)">
                  แก้ไข
                </v-btn>
              </template>
            </v-data-table>
        ...
        <v-card-actions class="justify-space-between">
          ...
          <v-btn color="primary" :disabled="!activeSettlement" @click="openBillPaymentDialog('create')">
            เพิ่มการชำระ
          </v-btn>
        </v-card-actions>
      </v-card>
    ...
    <v-dialog v-model="billPaymentDialogVisible" max-width="420">
      <v-card>
        ...
        <v-card-text>
          <div v-if="isBillPaymentEdit && billPaymentForm" class="payment-dialog-meta mb-4">
          ...
          <v-text-field label="Amount" v-model="billPaymentForm.amount" type="number" step="0.01" min="0"
            required></v-text-field>
          <v-text-field v-if="isBillPaymentCreate" label="Paid At" v-model="billPaymentForm.paid_at"
            type="datetime-local" :max="maxPaidAt" required></v-text-field>
          <v-select v-if="isBillPaymentCreate" label="Bank" :items="bankOptions" item-title="label" item-value="id"
            v-model="billPaymentForm.bank_id" clearable required></v-select>
        </v-card-text>
        <v-card-actions class="justify-end">
          <v-btn variant="text" @click="closeBillPaymentDialog">ยกเลิก</v-btn>
          <v-btn color="primary" :disabled="!canSubmitBillPayment || savingBillPayment" :loading="savingBillPayment"
  ...
  data() {
    return {
      ...
      headers: [
        ...
        { title: "มูลค่าส่วนลด", align: "end", key: "discount" },
        { title: "ส่วนลด", align: "end", key: "discount_name" },
  ...
  _methods: {
    exportExcel() {
      Swal.fire({
        ...
        if (re.isConfirmed) {
          const title = `รอบบิลที่ ${this.selectedWeekText} รวม ${this.totalHours} ชั่วโมง ${this.totalRealPrice} บาท`;
          const filename = `รอบบิลที่ ${this.selectedWeekText}`;

          const data = this.showItem.map((item) => {
            return {
              ลำดับ: item.No,
              ...
              ราคาส่วนลด: item.discount,
              ส่วนลด: item.discount_name,
              ประเภทส่วนลด: item.is_matchday,
              ราคาสุทธิ: item.net_price,
              ประกัน: item.insurance,
              ค่าธรรมเนียม: item.fee,
              มัดจำ: item.deposit_amount,
              ค่าบริการ: item.service_price,
              สรุป: item.sumary_all,
            };
          });

          exportExcelFun({ data, title, filename, sheetname: "sheet" });
        }
      });
    },
    formatDate(dateStr) {
    ...
    async searchMonthly() {
      if (this.selectedWeek) {
        ...
        if (selectedWeekData) {
          ...
          const response = await ConsoleService.getMatchV2(startTime, endTime);
    ...
    async onWeekSelected(selectedWeek) {
      try {
        ...
        const response = await ConsoleService.getMatchV2(
          startTime,
          endTime,
          providerId
        );
    ...
    showItem() {
      ...
      return sortedResponse.map((item, index) => {
        const startTime = moment(item.time_start)
          .subtract(1, "minutes")
          .format("YYYY-MM-DD HH:mm:ss");
        const endTime = moment(item.time_end).format("YYYY-MM-DD HH:mm:ss");
        ...
        // Use bill_order from API V2 structure
        ...
        if (hasPromotion) {
          ...
          // ไม่มีส่วนลด: ใช้ net_price - insurance
          realPrice = netPrice - insurance;
        }

        // หัก insurance ออกจาก netPrice สำหรับแสดงในตาราง
        ...
        return {
          ...
          sumary_price: item.total_price + insurance,
